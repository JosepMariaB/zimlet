...
com_irontec_zsugarH.prototype._moreOptionsDialog = function() {
		...
		newOpportunityButton.setImage("ISUGAR-lead");	
		...
		newCasesButton.setImage("ISUGAR-lead");	
		...
		newContactButton.setImage("ISUGAR-lead");	
...
com_irontec_zsugarH.prototype._AccountCreated = function(response){
	if (response.id) {
		var url =  this.getUserPropertyInfo("my_zsugar_url").value+"/index.php?module=Accouns&action=DetailView&record="+response.id;
		var msg = this.getMessage("zsugar_leadCreated")+"<br><a target='_blank' href='"+url+"'>"+this.inAccountFirstNameBox.getValue()+"</a>";
		appCtxt.getAppController().setStatusMsg(msg);
		return this._updateCRMValue();
	} else {
		return appCtxt.getAppController().setStatusMsg(this.getMessage("zsugar_leadError"));
